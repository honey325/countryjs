  ...
  var _returnCountry = function (country, type) {
    if (type === "name") {
      ...
      return _.find(_countryList, function (thiscountry) {
        return normalizeName(thiscountry.ISO.alpha3) == country;
      });
    } else {
      return _.find(_countryList, function (thiscountry) {
        let altSpellings = false;
        _.forIn(thiscountry.altSpellings, function (value) {
          if (normalizeName(value) == country) {
            altSpellings = true;
          }
        });
        return (
          normalizeName(thiscountry.ISO.alpha2) == country ||
          normalizeName(thiscountry.name) == country ||
          altSpellings ||
          normalizeName(thiscountry.ISO.alpha3) == country
        );
      });
    }
  };
  ...
  _.forEach(
    ...
    function (property, method) {
      this[method] = function (country, type) {
        var _returnData = _returnCountry(normalizeName(country), type);
        if (_returnData) {
          if (property == "altSpellings") {
            const array = _returnData[property];
            array.push(_returnData["name"]);
            array.push(_returnData.ISO.alpha3);
            array.push(_returnData.ISO.alpha2);
            return array;
          } else if (property) {
